...
from scripts.papers.bl_pv import plotdir

path = user_variables.datadir + 'iop8/'

track_x = np.load(path + 'track_x.npy')
track_y = np.load(path + 'track_y.npy')
...
def main(cubes):
    # fig = overview()
    # fig.savefig(plotdir + 'iop8_no_lw_overview_' + str(time) + 'h.png')
    ...
    for key in keys:
        ...
        fig.savefig(plotdir + 'iop8_cross_section_all_' +
                    key + '_' + str(time) + 'h.pdf')
...
def plot_overview(mslp, pv):
    ...
    plt.plot(track_x, track_y, '-y')
    plt.plot(track_x[time], track_y[time], 'yx', markersize=15)

    print(track_x[time], track_y[time])
