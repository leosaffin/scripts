...
def main():
    ...
    # BL relative
    """
    coord = 'distance_from_atmosphere_boundary_layer_height'
    ylabel = 'Vertical distance from boundary-layer top (km)'
    xlims = [(0.1, 0.8), (-0.7, 0.7), (-0.7, 0.7)]
    ylims = (-1, 1)
    profile(coord, mappings, domains, title, xlabel, ylabel, xlims, ylims)
    plt.savefig(plotdir + 'ch7_low/bl_profile.pdf')
    plt.show()
    """
    return
...
def profile(coord, mappings, domains, title, xlabel, ylabel, xlims, ylims):
    ...
    for m, domain in enumerate(domains):
        ...
        # cubes = iris.load('/home/lsaffin/Documents/meteorology/' +
        #                  'data/season/run_3/bl_profile_full.nc')
        for n, mapping in enumerate(mappings):
            ...
            plot.legend(ax, key=second_analysis.get_idx,
                        loc='upper left', ncol=2, bbox_to_anchor=(0.05, -0.1))
